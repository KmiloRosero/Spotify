...
class DoublyLinkedList {
    ...
    addFirst(data) {
        ...
        const node = new Node_1.ListNode(data);
        if (!this.head) {
            this.head = node;
            this.tail = node;
            this.length = 1;
    ...
    insertAt(index, data) {
        ...
        if (index < 0 || index > this.length) {
            throw new Error(`Index out of bounds. Received ${index}, but valid range is 0 to ${this.length}.`);
        }
        if (index === 0) {
            this.addFirst(data);
            return;
    ...
    getNodeAt(index) {
        if (index < 0 || index >= this.length) {
            throw new Error(`Index out of bounds. Received ${index}, but valid range is 0 to ${Math.max(0, this.length - 1)}.`);
        }
